...
extern uint8_t arri_buffer[10]; // this variable use to extract the data from uart recieved data uart4
extern uint8_t orignal_data; // this is variable to recieve the data from uart4
	...
	void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)  
{
    ...
    if (huart->Instance == UART5)  //current UART
		...
		{
			if(num_index==0) // if there is no character recived 
			{
			for(i=0;i<=100;i++)
				{arri_buffer[i]=0;
				}
			}
				 if (orignal_data!='\r')               // if the character are recieve other than enter or ASCII "13"
				 {
		      arri_buffer[num_index++]=orignal_data;    //here the data are fatch to the buffer
				 }
				 else
				{
				num_index=0;
	    ...
	    {
			HAL_UART_Receive_IT(&huart4,(uint8_t *)&orignal_data, 1); //put the uart in recieve mode in order to recieve the next packet
		  } 
	
}
}
void transimit_str_to(uint8_t board,uint8_t * str,uint8_t strlen) //funtion defination for UART5
{
